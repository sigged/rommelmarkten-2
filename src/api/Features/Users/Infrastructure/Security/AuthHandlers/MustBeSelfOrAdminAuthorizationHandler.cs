...
{
    public class MustBeSelfOrAdminRequirement : AuthorizationHandler<MustBeSelfOrAdminRequirement, UserProfile>, IAuthorizationRequirement
    {
        public override async Task HandleAsync(AuthorizationHandlerContext context)
        {
            if (!(context.Resource is UserProfile))
            {
                throw new InvalidOperationException($"Resource in {nameof(MustBeSelfOrAdminRequirement)} must be of type {nameof(UserProfile)}");
            }
            else
            {
